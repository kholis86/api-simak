...
class akmControllerCopy extends Controller
{
            ...
            try {
                ...
                $processedData = $data
                    ...
                    ->map(function ($studentSemesters) use ($activeTermYear) {
                        ...
                        return $studentSemesters->map(function ($item) use (&$allCumulativeSks, &$cumulativeSks, &$cumulativeBnkTotal, &$previousIps, $activeTermYear) {
                    ...
                    ->map(function ($studentsBySemester) {
                        return $studentsBySemester->map(function ($item) {
                            return [
                                ...
                                'nim'           => $item->Nim,
                                'sks'           => $item->sks,
                                'sks_total'     => $item->sks_total, // Field sks_total baru (Kumulatif)
                                'bnk_total'     => $item->bnk_total,
                                'ips'           => $item->ips, // Ganti dari 'ipk' menjadi 'ips'
                                'ipk'           => $item->ipk, // Field IPK baru (Kumulatif)
                                'department_id' => $item->Department_Id,
    ...
    public function akmPoint1(Request $request) {}
}
